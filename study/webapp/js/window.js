//默认只要有url我就给他加iframe，这样他就不能配置了
//jQuery在定义变量时，default中的最后一个变量后边千万别加逗号，否则IE6会出错。。。。就是maxSize
(function ($) {
	 $.fn.extend({
		window: function(opts){
			var defaults = {
				//用户只能定义整体的宽度和高度,其他的用户都不能自定义
			...
			//var $cusWindow_icon = $('<div class="close_window"></div><div class="max_size"></div><div class="min_size"></div>');
			...
			function init() {
				if(defaults.id != "") {
					...
					//这里的self一定要注意是在加载到body之后再去取，否则找不到该id所代表的html元素
					self = $("#"+defaults.id);
					setPositionCenter();
					if(defaults.url != ""){
						if(defaults.hasIFrame == true) {
					...
					$allShadow.width($(document).innerWidth()).height($(document).height());
			...
			function resizeTop(e){
				...
				function resizingTop(e) {
					...
					defaults.hasIFrame == true ? $ifr.css({"height":vHeight - 30}) : 5;	
					return false;
				}	
			}
			...
			function loadResize() {
				$( window ).resize(function() {
					...
					var allWidth = 0;
					//这里需要把所有的窗口循环一边,让最小话的窗口的位置跟着改变包含top和left
					$(".cusWindow").each(function(){
						if($(this).attr("minsize") == "true") {
							$(this).css({"top":top});
					...
					//上边的代码是对最小化的窗口进行重新排列，让他们的样式跟着一块变小	
