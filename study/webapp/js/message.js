...
function msgFailed(title, html, callback) {
    msgInstance(3, true, title, html, callback);
}
...
(function($){
    ...
    $.fn.extend({
        ...
        messager: function(options, value) {
            //Close existed messager by argument "close"
            if(options === "close")
            ...
            if(self.prop("tagName") == "FORM") {
                //todo: handle form submit confirm
                /*self.bind('submit', self._msgInit);*/
        ...
        _msgContent: function() {
            ...
            switch(msgOptions.type) {
                ...
                break;
                case 1:
                    $( msgIcon ).html("?").css({ 'color': "#ff8c00" });
                break;
                case 2:
					$(msgIcon).addClass("success");
                    //$( msgIcon ).html("✔").css({ 'color': "green" });
                break;
                case 3:
                    $( msgIcon ).html("✘").css({ 'color': "red" });
                break;
            ...
            var msgContent = $( '<div></div>' ).appendTo(messager).addClass("container")
				...
				.append($('<div class="msg-content"></div>')
            ...
            if(msgOptions.linkToClose) $( msgContent ).find("a, input").bind('click', function() { self._msgClose(); });
        },
        _msgButtons: function() {
            ...
            /*if (msgOptions.headerCloser && !msgOptions.confirm)
                $( '<a href="."></a>' ).html("X").addClass("closer").bind('click', self._msgClose).appendTo(msgHeader);*/
        },
