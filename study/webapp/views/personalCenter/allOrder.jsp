...
<body>
	<header id="header_bar">
    	<div class="operate">
            ...
            <div class="list" style="display:inline;">全部订单<i></i>
            	<div class="orderInfo">
    ...
    <article class="page_content">
    	<section class="step1">
          <c:forEach var="item" items="${orderInfo}"> 
	          <div class="numIndent">
	          	<input type="hidden" value="${fn:substring(item.key, 0, fn:length(item.key)-2)}"/>	
				<c:if test="${fn:substring(item.key, fn:length(item.key)-1, fn:length(item.key)) eq '4'}">
	           		<input type="button" value="去评价" class="content_button btn_remark">
	           	</c:if>
	           	<c:if test="${fn:substring(item.key, fn:length(item.key)-1, fn:length(item.key)) eq '3'}">
	           		<input type="button" value="确认收货" class="content_button" onclick="shouhuo(${fn:substring(item.key, 0, fn:length(item.key)-2)});">
	           	</c:if>
	          ...
	          <c:forEach var="it" items="${item.value}"> 
		          	<div class="cartCon dfk">
						...
						<c:if test="${it.remark eq '1'}">
							<p class="explain">等待付款</p>
						</c:if>
						<c:if test="${it.remark eq '2'}">
							<p class="explain">等待发货</p>
						</c:if>
						<c:if test="${it.remark eq '3'}">
							<p class="explain">确认收货</p>
						</c:if>
						<c:if test="${it.remark eq '4'}">
    ...
    <script src="/jiuyi/scripts/allOrder.js"></script>
